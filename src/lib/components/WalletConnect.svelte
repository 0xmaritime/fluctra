  ...
  {#if isDropdownOpen}
    ...
    >
      ...
      <div class="p-2">
        ...
        {#if isConnected}
          <div class="mt-2 pt-2 border-t border-[var(--color-border-default)]">
            <button
              on:click={() => {
                handleToggle(); // FIX: Call renamed prop
                isDropdownOpen = false;
              }}
              class="w-full flex items-center px-3 py-2 text-sm text-[var(--color-error-500)] hover:bg-[var(--color-error-100)] dark:hover:bg-[color-mix(in_oklch,var(--color-error-700)_30%,transparent)] rounded-lg transition-colors duration-200"
              >
              <span class="mr-2">
