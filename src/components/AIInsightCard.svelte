...
<div class={cardClasses}>
  <div class="flex items-center space-x-2 mb-[calc(var(--spacing)*2)]"> <!-- mb-4 -> 16px -->
  ...
  {#if insights.length > 0}
    <ul class="space-y-2 mb-[calc(var(--spacing)*3)]"> <!-- mb-6 -> 24px -->
      {#each insights as insight}
        <li class="flex items-start">
          <span class="text-[var(--color-secondary-500)] mt-[calc(var(--spacing)/4)] mr-2"> <!-- mt-1 (adjust tiny offset), text-secondary-500 -->
  ...
  {:else}
    <p class="text-[var(--color-text-secondary)] mb-[calc(var(--spacing)*3)]">No insights available</p> <!-- mb-6 -> 24px, text-neutral-700 -> text-secondary -->
  {/if}
